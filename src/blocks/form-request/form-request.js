...
// form-request--excursion
if ($("#form-excursion").length){
  $(function() {
    document.getElementById('form-excursion').addEventListener('submit', function(evt){
      var http = new XMLHttpRequest(), f = this;
      var th = $(this);
      evt.preventDefault();
      http.open('POST', './send-excursion.php', true);
      http.onreadystatechange = function() {
        if (http.readyState == 4 && http.status == 200) {
          th.find('input').val('');
          th.trigger("reset");
